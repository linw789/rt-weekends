...
The intuition is that the values of the probabilities of the [a, $y_0$], [a, $y_1$], ..., [a, $y_{99}$]
...
close to a flat line (a constant value in the 2D case).
...
For Lambertian surface, the scattering PDF: $pScatter(x, w_i, w_o, \lambda) = C \cdot cos(\theta_o)$ should read:
regardless of incident direction, the wavelength of the light, or the scattering position, the directional
distribution function of the reflected direction over solid angle is a function of the outgoing direction's angle
relative to the surface normal. Even though there are two variables $\theta$ and $\phi$ in spherical coordinates,
$\phi$ doesn't affect the probability of the reflected direction.

Here the reflected direction is the incident direction because we are tracing rays from the pixels back to the
light sources.

## How could the probability distribution of the reflected (incident) direction depend on the incident direction?

$pScatter(x, w_i, w_o, \lambda)$ describes the directional distribution of the reflected ray. But the reflected
(incident) ray itself is also a variable input to this PDF. How could it be?
...
Generating a random direction is equivalent to picking a random point on the surface of unit sphere. Unlike
integration in cartesian coordinates, the differential area on a surface in spherical coordinates is:
$dA = sin(\theta)d\theta d\phi$. The $sin(\theta)$ exists because the area per unit $\theta$ around the equator
is larger than that around the poles (the circumference of the circle perpendicular to the z-axis is larger).

So given a PDF $p(\theta, \phi)$, we can calculate the probability of a point falling in a differential area
on the surface as: 

$$ p(\theta, \phi) sin(\theta) d\theta d\phi $$


Say we have a random point generator under the distribution PDF $p(\theta, \phi) = f(\theta)$, we can get the 1D
PDFs for both spherical coordinates $\theta$ and $\phi$. This is called [marginal distribution](https://en.wikipedia.org/wiki/Marginal_distribution). 
To get 1D PDF of $\theta$, we sum joint probability (probability of a specific pair of $\theta$ and $\phi$) over
all values of $\phi$.
...
The book doesn't say what $\theta$ is. [ScratchAPixel](https://www.scratchapixel.com/lessons/3d-basic-rendering/introduction-to-lighting/introduction-to-lighting-area-lights-theory.html)
in the following picture explains that $\theta$ here is the angle between the incident ray and the normal of the area
light plane.

![Area Light PDF](https://www.scratchapixel.com/images/introduction-to-lighting/intro-lighting-spherical-polygon2.png)
